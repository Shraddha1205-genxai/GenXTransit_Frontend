...
export interface FarePoliciesPageProps {
  data?: FarePolicy[];
  routeOptions?: string[];
...
function Badge({ children, tone }: { children: React.ReactNode; tone?: "green" | "amber" | "red" }) {
  const colorMap = {
    green: { bg: T.greenFill, text: T.green },
    amber: { bg: T.amberFill, text: T.amberDeep },
    red: { bg: T.red, text: "#fff" },
  };
  const style = colorMap[tone || "amber"];
  return <span style={{ display: "inline-flex", alignItems: "center", padding: "3px 8px", borderRadius: 999, fontSize: 11, fontWeight: 700, background: style.bg, color: style.text }}>{children}</span>;
}
...
function Modal({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode }) {
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(16,27,38,0.55)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 20 }} onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: T.panel, borderRadius: 8, width: 500, maxWidth: "100%", maxHeight: "85vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(16,27,38,0.35)" }}>
...
export function FarePoliciesPage({ data: propData, routeOptions = [], onAdd, onUpdate, onDelete }: FarePoliciesPageProps) {
  ...
  return (
    <div>
      ...
      >
          ...
          <tbody>
            {data.map((item) => (
              <tr key={item.code} className="stc-row">
                ...
                <Td><Badge tone={item.status === "Published" ? "green" : item.status === "Simulated" ? "amber" : "red"}>{item.status}</Badge></Td>
                <Td align="right">
        ...
        {modal && (
          <Modal title={`${modal.mode === "add" ? "Add" : "Edit"} — Fare Policies`} onClose={() => setModal(null)}>
...
export default FarePoliciesPage;
