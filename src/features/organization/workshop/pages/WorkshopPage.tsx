...
export function WorkshopPage({ data: propData, depotOptions = [], onAdd, onUpdate, onDelete }: WorkshopPageProps) {
    ...
    <div>
          ...
          <tbody>
            {data.map((item) => (
              <tr key={item.code} className="stc-row">
                ...
                <Td>{item.bays}</Td>
                <Td align="right">{item.activeJobs}</Td>
                <Td align="right">
      ...
      {modal && (
        <Modal title={`${modal.mode === "add" ? "Add" : "Edit"} — Workshop`} onClose={() => setModal(null)}>
          ...
          <div style={{ marginBottom: 12 }}>
            <label style={{ display: "block", fontSize: 11, fontWeight: 600, color: T.textSoft, textTransform: "uppercase", marginBottom: 5 }}>Bays</label>
            <input type="number" style={{ width: "100%", padding: "8px 10px", border: `1px solid ${T.border}`, borderRadius: 4, fontSize: 13, boxSizing: "border-box" }} value={formData.bays ?? 0} onChange={(e) => setFormData((s) => ({ ...s, bays: Number(e.target.value) }))} />
          </div>
          <div style={{ marginBottom: 12 }}>
            <label style={{ display: "block", fontSize: 11, fontWeight: 600, color: T.textSoft, textTransform: "uppercase", marginBottom: 5 }}>Active jobs</label>
            <input type="number" style={{ width: "100%", padding: "8px 10px", border: `1px solid ${T.border}`, borderRadius: 4, fontSize: 13, boxSizing: "border-box" }} value={formData.activeJobs ?? 0} onChange={(e) => setFormData((s) => ({ ...s, activeJobs: Number(e.target.value) }))} />
          </div>
